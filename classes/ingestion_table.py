...
class IngestionTable(ADACTable):
    ...
    @property
    def is_full_ingestion(self) -> bool:
        return self._is_full_ingestion

    def get_deleted_data(self, partition: Dict[str, str]) -> DataFrame:
        return self.get_partition(partition).where(self["catulz_reg"] == "D")

    def get_most_recent_data(self, data_frame: DataFrame = None) -> DataFrame:
        return get_most_recent_data(
            data_frame if data_frame is not None else self,
            self.table_keys,
